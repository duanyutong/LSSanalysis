# this is the save directory; must contain "2Dbaofits" folder
...
Hdir = '/home/dyt/analysis_data/dr12/'  # this is where results are stored
...
#        Chioff = 0
    ...
    def chi_templ_alphfXXnA(self,list,fw='',wo='n'):
#        from time import time
#        t = time()
        BB = list[0]
        if BB < 0:
...
#        Chioff = 0
    ...
    def chi_templ_alphfXXnA0(self,list,fw='',wo='n'):
#        from time import time
#        t = time()
        BB = list[0]
        if BB < 0:
...
#        Chioff = 0
    ...
    def chi_templ_alphfXXnA2(self,list,fw='',wo='n'):
#        from time import time
#        t = time()
        BB = list[0]
        if BB < 0:
...
#        Chioff = 0
    ...
    def chi_templ_alphfXXW(self,list,fw='',wo='n'):
#        from time import time
#        t = time()
        BB = list[0]
        if BB < 0:
...
#        Chioff = 0
        ...
        chit0 = np.dot(np.dot(dl0,self.invt0),dl0)
        chit2 = np.dot(np.dot(dl2,self.invt2),dl2)
        BBfac = (np.log(BB/self.BB)/self.Bp)**2.
        Btfac = (np.log(Beta/self.Bta)/self.Bt)**2.
...
#    A22 = 0
    ...
    for i in range(0,nar):      
        ...
        for j in range(0,nat):
            ...
            fg.write(str(chi)+' ')
            if chi < chim:
                print('b.ar, b.at, chi: ', b.ar,b.at,chi)
                chim = chi
                alrm = b.ar
                altm = b.at
                Bm = B
    ...
    print('alrm, altm, chim: ', alrm,altm,chim) #,alphlk,likm
...
#    import sys
    ...
    rmaxb = 80. #the maximum scale to be used to set the bias prior
    ...
    # read in data
    # combined monopole, quadrupole cov matrix
    c = np.loadtxt(datadir+ft+zb+'covariance_monoquad_'+bc)
    # common r bin centre
    r_bins_centre = np.loadtxt(datadir+ft+zb+'correlation_function_monopole_'+bc)[:, 0]
    # monopole xi data vector
    d0 = np.loadtxt(datadir+ft+zb+'correlation_function_monopole_'+bc)[:, 1]
    # quadrupole xi data vector
    ...
    mini = 0 #minimum index number to be used for covariance matrix index assignment
    ...
    for i in range(len(c)):
        ...
        if i < len(d0):
            ...
            # ri = r_bins_centre[i-len(d0)]
            indi = int(len(dv)/2+i-mini-len(d0))
        ...
        for j in range(len(c)):
            ...
            if j < len(d0):
                ...
                indj = int(len(dv)/2+j-mini-len(d0))
    ...
    for i in range(0,len(dvb)):
        if i < len(dvb)/2:
            ...
            indi = int(i-len(dvb)/2+len(covm)/2)
        for j in range(0,len(dvb)):
            if j < len(dvb)/2:
                indj = j
            else:
                indj = int(j-len(dvb)/2+len(covm)/2)
